...
export class PdmTaskProvider implements TaskProvider {
	...
	public async resolveTask(_task: Task): Promise<Task | undefined> {
		...
		if (pdmTask) {
			...
			if (kind.path) {
				packageJsonUri = _task.scope.uri.with({ path: _task.scope.uri.path + '/' + kind.path + `${kind.path.endsWith('/') ? '' : '/'}` + 'pyproject.toml' });
			} else {
				packageJsonUri = _task.scope.uri.with({ path: _task.scope.uri.path + '/pyproject.toml' });
			}
...
export async function getPackageManager(extensionContext: ExtensionContext, folder: Uri, showWarning: boolean = true): Promise<string> {
	let packageManagerName = workspace.getConfiguration('pdm', folder).get<string>('packageManager', 'pdm');
		...
		if (showWarning && multiplePMDetected && !extensionContext.globalState.get<boolean>(neverShowWarning)) {
			...
			window.showInformationMessage(multiplePMWarning, learnMore, neverShowAgain).then(result => {
				switch (result) {
					...
					case learnMore: env.openExternal(Uri.parse('https://docs.pdmjs.com/cli/v9/configuring-pdm/package-lock-json'));
				}
			});
		}
	}
			...
			if (isAutoDetectionEnabled(folder) && !excludeRegex.test(Utils.basename(folder.uri))) {
				...
				for (const path of paths) {
					if (!isExcluded(folder, path) && !visitedPackageJsonFiles.has(path.fsPath)) {
						const tasks = await providePdmScriptsForFolder(context, path, showWarning);
	...
	try {
		...
		for (const path of paths) {
			if (!visitedPackageJsonFiles.has(path.fsPath)) {
				const tasks = await providePdmScriptsForFolder(context, path, true);
				visitedPackageJsonFiles.add(path.fsPath);
...
export function isAutoDetectionEnabled(folder?: WorkspaceFolder): boolean {
	return workspace.getConfiguration('pdm', folder?.uri).get<AutoDetect>('autoDetect') === 'on';
}
...
function isExcluded(folder: WorkspaceFolder, packageJsonUri: Uri) {
	...
	const exclude = workspace.getConfiguration('pdm', folder.uri).get<string | string[]>('exclude');
...
async function providePdmScriptsForFolder(context: ExtensionContext, packageJsonUri: Uri, showWarning: boolean): Promise<ITaskWithLocation[]> {
	const emptyTasks: ITaskWithLocation[] = [];
	...
	if (!workspace.getConfiguration('pdm', folder).get<string[]>('scriptExplorerExclude', []).find(e => e.includes(INSTALL_SCRIPT))) {
...
export async function createTask(packageManager: string, script: IPdmTaskDefinition | string, cmd: string[], folder: WorkspaceFolder, packageJsonUri: Uri, scriptValue?: string, matcher?: any): Promise<Task> {
	...
	function getCommandLine(cmd: string[]): (string | ShellQuotedString)[] {
		...
		if (workspace.getConfiguration('pdm', folder.uri).get<boolean>('runQuiet')) {
	...
	function getRelativePath(packageJsonUri: Uri): string {
		...
		const absolutePath = packageJsonUri.path.substring(0, packageJsonUri.path.length - 'pyproject.toml'.length);
	...
	const task = new Task(kind, folder, taskName, 'pdm', new ShellExecution(packageManager, getCommandLine(cmd), { cwd: cwd }), matcher);
...
async function hasRootPackageToml(): Promise<boolean> {
	...
	for (const folder of folders) {
		if (folder.uri.scheme === 'file') {
			const packageJson = path.join(folder.uri.fsPath, 'pyproject.toml');
...
export async function getScripts(packageJsonUri: Uri) {
	...
	// 	return readScripts(document);
	// } catch (e) {
	// 	const localizedParseError = `Pdm task detection: failed to parse the file ${packageJsonUri.fsPath}`;
	// 	console.error(localizedParseError);
